...
internal partial class VideoController
{
...
internal partial class VideoController
{
    public VideoWatchPage GetVideoWatchPage(
        VideoId videoId,
        CancellationToken cancellationToken = default
    ) {
        for (var retriesRemaining = 5;; retriesRemaining--)
        {
            var watchPage = VideoWatchPage.TryParse(
                Http.GetStringAsync($"https://www.youtube.com/watch?v={videoId}&bpctr=9999999999", cancellationToken).Result
            );
    ...
    public PlayerResponse GetPlayerResponse(
        ...
        using var response = Http.SendAsync(request, cancellationToken).Result;
        response.EnsureSuccessStatusCode();
